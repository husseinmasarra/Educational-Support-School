...
// Neon PostgreSQL Serverless Query Proxy API (Bypasses Browser CORS)
app.post('/api/neon/query', async (req, res) => {
  try {
    const { query, connectionUrl } = req.body;
    if (!query) {
      return res.status(400).json({ error: 'Missing query in payload' });
    }

    const connStr = connectionUrl || process.env.NEON_DATABASE_URL || 'postgresql://neondb_owner:npg_dMHDq2Ae1FXR@ep-billowing-recipe-awh6dwin.c-12.us-east-1.aws.neon.tech/neondb?sslmode=require';

    const response = await fetch('https://ep-billowing-recipe-awh6dwin.c-12.us-east-1.aws.neon.tech/sql', {
      method: 'POST',
      headers: {
    ...
    console.error('[Server Neon Proxy Error]:', err.message);
    res.status(500).json({ error: err.message });
  }
});
