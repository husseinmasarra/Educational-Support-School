      ...
      <div className="w-full px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2 sm:gap-4">
          ...
          <div 
            ...
            className="hidden lg:flex items-center gap-2 bg-[#0284C7]/20 hover:bg-[#0284C7]/40 border border-sky-400/30 px-3.5 py-2 rounded-full cursor-pointer transition-all text-slate-100 min-w-0 max-w-xs flex-1 select-none shadow-inner"
            title={lang === 'ar' ? 'البحث الذكي الشامل... (Ctrl+K)' : 'Smart search... (Ctrl+K)'}
          >
            <Search className="w-4 h-4 text-sky-200 shrink-0" />
            <span className="text-xs text-slate-200 font-bold truncate text-right grow">
              {lang === 'ar' ? 'البحث الذكي الشامل... (Ctrl+K)' : 'Smart search... (Ctrl+K)'}
            </span>
          </div>

          {/* Left Actions: Profile, App Download, Bell, Logout */}
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            
            {/* Logged-In User Profile Card (Large screens only) */}
            {currentUser && (
              <div 
                onClick={() => setShowAvatarModal(true)}
                className="hidden xl:flex items-center gap-2 bg-[#0284C7]/20 hover:bg-[#0284C7]/35 border border-[#0EA5E9]/20 p-1 pe-3 rounded-2xl transition-all cursor-pointer group shrink-0"
              ...
              >
                ...
                <div className="text-right rtl:text-right ltr:text-left text-xs max-w-[100px]">
                  <span className="font-bold text-white block group-hover:text-sky-300 transition-colors truncate">
            ...
            <a
              ...
              className="flex items-center gap-1 bg-[#EF4444] hover:bg-red-600 text-white px-2.5 py-1.5 rounded-xl text-xs font-black shadow transition-all cursor-pointer border border-red-400 shrink-0"
            ...
            >
              ...
              <span className="whitespace-nowrap">{lang === 'ar' ? 'تطبيق (APK)' : 'APK App'}</span>
            </a>

            {/* Mobile/Tablet Smart Search Trigger Button */}
            <button
              onClick={() => setIsSearchOpen(true)}
              className="lg:hidden p-1.5 sm:p-2 rounded-xl bg-[#0284C7]/20 hover:bg-[#0284C7]/40 border border-[#0EA5E9]/20 text-white transition-all cursor-pointer shrink-0"
