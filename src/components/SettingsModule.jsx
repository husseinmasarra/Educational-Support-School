...
export const SettingsModule = () => {
 ...
 <div>
 <h2 className="text-xl font-bold text-[#0284C7]">{t('settingsTitle')}</h2>
 <p className="text-xs text-slate-500 mt-1">
 {isAr 
 ? "إدارة إعدادات المدرسة، الشعار، أوقات الاستراحة، الربط السحابي، والنسخ الاحتياطي."
 : "Manage school identity, recess schedule, cloud database, and backup."}
 </p>
 </div>
 </div>
 ...
 >
 استعراض أرشيف الأعوام السابقة </button>
 </div>
 )}
 </div>
 ...
 Serverless PostgreSQL Ready </span>
 </div>
 ...
 <span>{isNeonLocked ? 'الرابط محمي ' : 'حفظ وقفل الاتصال '}</span>
  ...
  {/* Manual Live Cloud Sync Triggers */}
  <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-sky-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
    <div className="space-y-0.5">
      <h4 className="font-extrabold text-[#0284C7] flex items-center gap-1.5 text-sm">
        <span>مزامنة سريعة وتحديث يدوي مع السحابة ☁️</span>
      </h4>
      <p className="text-[11px] text-slate-500">
        اضغط رفع لنقل كافة بيانات الطلاب من هذا الجهاز لقاعدة البيانات السحابية فوراً، أو اضغط جلب للاستعادة على الهاتف.
      </p>
    </div>
    ...
    <div className="flex items-center gap-2 shrink-0">
      <button
        ...
        onClick={async () => {
          setIsSyncing(true);
          const res = await syncLocalToNeonCloud();
          setIsSyncing(false);
          if (res) {
            setToastMessage(isAr ? 'تم رفع كافة بيانات هذا الجهاز إلى قاعدة البيانات السحابية بنجاح!' : 'Pushed data to Cloud!');
          } else {
            alert(isAr ? 'تعذر الرفع، يرجى التحقق من الاتصال بالإنترنت' : 'Failed to push to Cloud');
          }
          setTimeout(() => setToastMessage(''), 3500);
        }}
        className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow transition-all cursor-pointer flex items-center gap-1.5"
      >
        <Upload className="w-4 h-4" />
        <span>{isSyncing ? 'جاري الرفع...' : 'رفع بيانات الجهاز للسحابة ☁️'}</span>
      </button>
      ...
      <button
        ...
        onClick={async () => {
          setIsSyncing(true);
          const success = await syncNeonCloudToLocal();
          setIsSyncing(false);
          if (success) {
            setToastMessage(isAr ? 'تم جلب وتحديث أحدث البيانات من قاعدة البيانات السحابية بنجاح!' : 'Fetched data from Cloud!');
          } else {
            alert(isAr ? 'لم يتم العثور على بيانات سحابية جديدة أو تعذر الاتصال' : 'No cloud data found');
          }
          setTimeout(() => setToastMessage(''), 3500);
        }}
        className="px-4 py-2.5 bg-[#0284C7] hover:bg-[#0369A1] text-white rounded-xl text-xs font-bold shadow transition-all cursor-pointer flex items-center gap-1.5"
      >
        <Download className="w-4 h-4" />
        <span>{isSyncing ? 'جاري التحميل...' : 'جلب واستعادة من السحابة 🔄'}</span>
      </button>
    </div>
  </div>
 ...
 >
 <Calendar className="w-4 h-4" />
 تأكيد وبدء العام الدراسي الجديد </button>
 </div>
 </div>
 </div>,
 document.body
 ...
 أرشيف الأعوام الدراسية السابقة المحفوظة </h3>
