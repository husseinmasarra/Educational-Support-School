...
export const StudentCardPage = () => {
 ...
 {isAr ? 'يمكنك إضافة طالب جديد عبر قسم (دليل المدرسة) لطباعة بطاقته الرقمية المعتمدة.' : 'Add a student from Directory to preview student ID card.'}
  ...
  const baseTuition = student?.isSpecialCase ? 0 : Number(student?.tuitionTotal || 0);
  const adminFees = student?.isSpecialCase ? 0 : Number(student?.adminFees || 0);
  const transportFee = student?.hasTransport ? (Number(student?.transportFee) || 0) : 0;
  const totalTuition = student?.isSpecialCase ? 0 : Math.max(0, baseTuition + adminFees + transportFee);
  const discountTuition = student?.isSpecialCase ? 0 : Number(student?.tuitionDiscount || student?.discount || 0);
  const paidTuition = Number(student?.tuitionPaid || 0);
  const netTuition = student?.isSpecialCase ? 0 : Math.max(0, totalTuition - discountTuition);
  const remainingTuition = student?.isSpecialCase ? 0 : Math.max(0, netTuition - paidTuition);
 ...
 >
  ...
  {isAr ? s.name : s.nameEn} {s.isSpecialCase ? '⭐' : ''} ({isAr ? s.grade : s.gradeEn})
  </option>
  ))}
 </select>
 </div>
 ...
 {student.isSpecialCase && <span className="text-amber-500 font-bold text-sm" title="حالة خاصة">⭐</span>}
 </h3>

 <span className={`px-3 py-1 rounded-full text-xs font-bold ${
 student.isSpecialCase || remainingTuition === 0 ? 'bg-emerald-50 text-emerald-800 border border-emerald-300' : 'bg-red-50 text-red-800 border border-red-300'
 }`}>
 {student.isSpecialCase ? '⭐ حالة خاصة ($0)' : remainingTuition === 0 ? (isAr ? 'مسدد بالكامل ' : 'Fully Paid ') : (isAr ? 'يوجد قسط متبقي ' : 'Pending Balance ')}
 </span>
 </div>
 ...
 <span>{isAr ? student.name : student.nameEn}</span>
 {student.isSpecialCase && <span className="text-amber-400 font-bold text-sm" title="حالة خاصة">⭐</span>}
 </h4>
