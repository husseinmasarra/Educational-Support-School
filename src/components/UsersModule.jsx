...
export const UsersModule = () => {
 ...
 reader.readAsDataURL(file);
  ...
  const handleRoleChange = (role) => {
    ...
    if (role === 'admin') {
      ...
      setNewUserPermissions(['manage_all', 'manage_finance', 'add_student', 'manage_users', 'print_cards']);
    } else if (role === 'assistant_admin') {
      setNewUserRoleTitle('مساعد مدير');
      setNewUserPermissions(['add_student', 'manage_finance', 'send_messages', 'print_cards']);
 ...
 <option value="admin">{isAr ? 'المدراء (Admin)' : 'Admins'}</option>
 <option value="assistant_admin">{isAr ? 'مساعد مدير (Assistant Admin)' : 'Assistant Admins'}</option>
 ...
 <option value="assistant_admin">مساعد مدير (Assistant Admin)</option>
 ...
 <span>تعديل اسم الحساب، كلمة السر الصلاحيات </span>
 </h3>
 <button 
 type="button"
