...
export const systemPermissionOptions = [
  ...
  { id: 'add_student', name: 'إضافة وتعديل بيانات الطلاب والإخوة', nameEn: 'Add & Edit Students & Siblings', category: 'admin' },
  ...
  { id: 'send_messages', name: 'إرسال التنبيهات والرسائل المباشرة والتذكيرات', nameEn: 'Send Notifications & Messages', category: 'teacher' },
...
export const initialSystemUsers = [
  {
    ...
    permissions: ['manage_all', 'manage_finance', 'add_student', 'manage_users', 'send_lessons', 'manage_bus', 'print_cards']
  },
  {
    id: "USR-02",
    name: "مساعد المدير العام",
    nameEn: "Assistant Admin",
    username: "assistant",
    password: "123123123",
    ...
    permissions: ['add_student', 'manage_finance', 'send_messages', 'print_cards']
  }
];
...
export const AppProvider = ({ children }) => {
 ...
 return updatedUsers;
  ...
  const syncLocalToNeonCloud = async () => {
    ...
    return await saveToNeonCloud(dbPayload);
  };

  const syncNeonCloudToLocal = async () => {
    const cloudData = await loadFromNeonCloud();
    if (cloudData && typeof cloudData === 'object' && Array.isArray(cloudData.school_students) && cloudData.school_students.length > 0) {
  ...
  // 2. Periodic background cloud sync (checks for updates from other devices every 15s)
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        // silent fail on periodic background poll
      }
    }, 15000);
  ...
  // 3. Save database to server, Neon Cloud DB & localStorage whenever any collection changes
  useEffect(() => {
    if (isInitializingSync) return;
    ...
    // Save to dev server if available
    fetch('/api/db/save', {
      method: 'POST',
      headers: { 
    ...
    // Save to Neon PostgreSQL Cloud DB automatically
    saveToNeonCloud(dbPayload).catch(err => {
      console.error('[Neon Auto-Save Error]:', err);
    });
  }, [
    isInitializingSync,
    subjects,
 ...
 const isSibling = (s1, s2) => {
 if (!s1 || !s2) return false;
 if (s1.id && s2.id && s1.id === s2.id) return true;
 const p1Name = (s1.parentName || s1.parent_name || '').trim().toLowerCase();
 const p2Name = (s2.parentName || s2.parent_name || '').trim().toLowerCase();
 const p1Phone = (s1.parentPhone || s1.parent_phone || s1.phone || '').trim();
 const p2Phone = (s2.parentPhone || s2.parent_phone || s2.phone || '').trim();
 
 if (p1Name && p2Name && p1Name === p2Name) return true;
 ...
 const markSpecialCase = stuObj.isSpecialCase || hasSpecialCaseSibling;
 ...
 const newIsSpecial = updatedFields.isSpecialCase !== undefined ? updatedFields.isSpecialCase : targetStu.isSpecialCase;
 ...
 return {
 ...s,
 ...(s.id === studentId ? updatedFields : {}),
 isSpecialCase: newIsSpecial,
 tuitionTotal: newIsSpecial ? 0 : (s.id === studentId && updatedFields.tuitionTotal !== undefined ? updatedFields.tuitionTotal : s.tuitionTotal)
 };
 }
 return s;
 });
 ...
 syncLocalToNeonCloud,
 syncNeonCloudToLocal
 };
